...
pub async fn decode(args: DecodeArgs) -> Result<Vec<ResolvedFunction>, Error> {
    ...
    if args.explain && args.openai_api_key.is_empty() {
        error!("OpenAI API key is required for explaining calldata. Use `heimdall decode --help` for more information.");
        ...
        .map_err(|_| Error::GenericError("failed to match calldata regex.".to_string()))?
    {
        ...
        error!("invalid target. must be a transaction hash or calldata (bytes).");
        return Err(Error::GenericError(
    ...
    let byte_args = match decode_hex(&calldata[8..]) {
        ...
        Err(_) => {
            error!("failed to parse bytearray from calldata.");
            ...
            match decoded_function_call {
                Ok(decoded_function_call) => {
                        ...
                        .split_once(&function_selector.replace('0', ""))
                    {
                        ...
                        None => {
                            debug!("potential match '{}' ignored. decoded inputs differed from provided calldata.", &potential_match.signature);
                            continue;
                        }
                    };
