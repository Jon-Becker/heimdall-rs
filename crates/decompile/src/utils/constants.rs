...
lazy_static! {
    ...
    /// llm postprocessing prompt for entire contract
    pub static ref LLM_POSTPROCESSING_PROMPT: String =
"The following Solidity contract was generated by a decompiler. It is messy and lacks meaningful variable names and comments.
This code may not compile and may include invalid Solidity syntax.

Your task is to postprocess this entire decompiled contract, cleaning it up and making it more readable. Specifically:
 - Add comments where you are able to infer what the code does.
 - Rename variables where you can infer what the variable is used for (e.g., `var_a` to `amount`, `arg0` to `recipient`).
 - Remove redundant checks or lines that do not affect the overall functionality of the code.
 - Add an `@notice` to the end of each function's pre-existing docstring with a short summary of the function. Ensure the notice follows the indentation and spacing of the rest of the docstring.
 - Preserve all existing NatSpec documentation (@custom:selector, @custom:signature, @param annotations).

Return the complete annotated contract source code.
