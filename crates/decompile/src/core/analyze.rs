...
use heimdall_vm::ext::exec::VMTraceStorage;
use tracing::debug;
...
/// [`VMTraceStorage`] generated by symbolic execution.
...
impl Analyzer {
    ...
    pub(crate) async fn analyze(
        ...
        // Perform analysis starting from the root trace
        if let Some(root_id) = trace_storage.get_root_id() {
            self.analyze_trace(&trace_storage, root_id, &mut analyzer_state).await?;
        }
    ...
    /// Analyze a trace by its ID
    fn analyze_trace<'a>(
        &'a mut self,
        storage: &'a VMTraceStorage,
        trace_id: u64,
        analyzer_state: &'a mut AnalyzerState,
    ) -> BoxFuture<'a, Result<(), Error>> {
        Box::pin(async move {
            // Get the trace from storage
            let trace = match storage.get_trace(trace_id) {
                Some(t) => t,
                None => return Ok(()),
            };
            ...
            // for each operation in the current trace branch, perform analysis with registered
            // heuristics
            ...
            for child_id in &trace.children {
                self.analyze_trace(storage, *child_id, analyzer_state).await?;
            }
