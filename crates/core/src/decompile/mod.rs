...
pub async fn decompile(args: DecompilerArgs) -> Result<DecompileResult, Error> {
    ...
    if resolved_selectors.is_empty() {
        ...
        if args.include_yul {
            trace!("analyzing symbolic execution trace with yul analyzer");
            ...
            trace!("analyzing symbolic execution trace with sol analyzer");
    ...
    for (selector, function_entry_point) in selectors {
        ...
        let mut analyzed_function;
        if args.include_yul {
            trace!("analyzing symbolic execution trace '0x{}' with yul analyzer", selector);
            analyzed_function = analyze_yul(
                &map,
                Function {
            ...
            trace!("analyzing symbolic execution trace '0x{}' with sol analyzer", selector);
            analyzed_function = analyze_sol(
                &map,
                Function {
                    selector: selector.clone(),
